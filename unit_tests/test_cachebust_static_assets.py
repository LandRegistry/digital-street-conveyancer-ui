...
from conveyancer_ui.custom_extensions.cachebust_static_assets.main import md5_for_file
from conveyancer_ui.custom_extensions.cachebust_static_assets.main import CachebustStaticAssets
...
class TestCachebustStaticAssets(unittest.TestCase):
    ...
    @mock.patch('conveyancer_ui.custom_extensions.cachebust_static_assets.main.CachebustStaticAssets.init_app')
    ...
    def test_url_for_adds_cache_query_string(self):
        filename = 'conveyancer_ui/assets/dist/test.txt'
        with open(filename, 'w+') as file:
            file.write('Hello')
    ...
    @mock.patch('conveyancer_ui.custom_extensions.cachebust_static_assets.main.md5_for_file', wraps=md5_for_file)
        ...
        with app.test_request_context('/'):
            ...
            with self.assertLogs(level='ERROR') as logs:
                ...
                self.assertRegexpMatches(str(logs),
                                         'File not found: /.*/conveyancer_ui/assets/dist/doesnt-exist.txt')
    ...
    def test_far_future_expiry_headers_for_css_file(self):
        filename = 'conveyancer_ui/assets/dist/test.css'
        with open(filename, 'w+') as file:
            file.write('Hello')
